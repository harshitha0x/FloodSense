...
def render_map_view():
    ...
    with sidebar_col:
        ...
        if show_flood_zones or show_reservoirs:
            st.markdown('<div class="section-label" style="margin-top:16px">LEGEND</div>', unsafe_allow_html=True)
            st.markdown("""
    ...
    with map_col:
        ...
        if show_flood_zones:
            ...
            for zone in FLOOD_ZONES:
                ...
                folium.CircleMarker(
                    ...
                    fill_color=color,
                    fill_opacity=0.35,
                    popup=folium.Popup(f"<b style='color:{color}'>{name}</b><br>{risk} RISK<br>{desc}", max_width=220),
        ...
        if show_reservoirs:
            ...
            for r in RESERVOIRS:
                ...
                folium.Marker(
                    ...
                    popup=folium.Popup(f"<b style='color:{r['color']}'>💧 {r['name']}</b><br>{cap_text}<br>{r['note']}", max_width=240),
                    tooltip=f"💧 {r['name']}",
                    icon=folium.Icon(color="red" if r["risk"] == "HIGH" else "orange" if r["risk"] == "MEDIUM" else "blue", icon="tint", prefix="fa")
                ).add_to(res_group)
            res_group.add_to(m)
        ...
        if show_rain_heatmap:
            ...
            HeatMap(scaled_points, radius=35, blur=25, min_opacity=0.3).add_to(heat_group)
            heat_group.add_to(m)
        ...
        if show_evacuation:
            ...
            for ec in EVACUATION_CENTERS:
                folium.Marker(
                    ...
                    popup=folium.Popup(f"<b>{ec['name']}</b><br>{ec['type']} · {ec['capacity']:,} people", max_width=220),
                    tooltip=f"🚨 {ec['name']}",
